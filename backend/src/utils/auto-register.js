...
module.exports = {
    registerPlugins: (fastify,dir = "../plugins/**.js") => {
      glob.sync(path.join(__dirname, dir)).forEach((file) => {
         ...
         consola.success(`Added ${'plugin'.yellow} ${path.basename(file,'.js').green}`);
      });
    },
    registerRoutes: (fastify,dir = "../routes/**.js") => {
        ...
        glob.sync(path.join(__dirname, dir)).forEach((file) => {
            ...
            if("index" === route.trim().toLowerCase()) {
                ...
                fastify.register(require(file), {prefix: '/' + path.basename(file, '.js')});
                consola.success(`Added ${'route'.cyan} ${('/'+path.basename(file,'.js')).green}`);
            }
        });
    }
};